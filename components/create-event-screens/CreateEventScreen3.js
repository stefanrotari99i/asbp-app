import { SafeAreaView, StyleSheet, View, Text, TouchableOpacity, TextInput, Image, Animated, FlatList, Vibration} from "react-native"
...
const ItemDrink = ({item, onDalabaiob}) => {
    ...
    // reder swipe action on right
    const renderRightActions = () => {

        return (
            <TouchableOpacity style={css.rightaction} onPress={onDalabaiob}>
        ...
        <Swipeable renderRightActions={renderRightActions} >
            <View style={css.field}>
                <TouchableOpacity style={css.imginput} onPress={pickImage}>
                    ...
                    {image && <Image style={css.uploadimg} source={{ uri: item.image }} resizeMode={'cover'} /> }
                </TouchableOpacity>
                <View style={css.fieldwrapper}>
                    ...
                    <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                        <View style={css.countinput}>
                            <TextInput style={css.input} placeholder='Liters of drinks' placeholderTextColor={'rgba(255,255,255, .5)'} keyboardAppearance={'dark'} keyboardType={'number-pad'}/>
                        </View>
                    </View>
                </View>
            </View>
...
export const CreateEventScreen3 = ({navigation}) => {
    const [fieldList, setFieldList] = useState(DATA)



    const deleteItem = (item) => {
        const newList = fieldList.filter(i => i.id !== item.id)
    ...
    const onAddBtnClick = () => {
        setFieldList([...fieldList, {   'id': new Date(),   'name': 'Drink ' + (fieldList.length + 1),   'count': '1.5',   'image': 'https://images.unsplash.com/photo-1518791841217-8f162f1e1131?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1050&q=80' }])  ;
    };
    return(
        <View style={css.container}>
            ...
            <KeyboardAwareScrollView style={{flex: 1}}>
                <View style={{width: '90%', alignItems: 'center', alignSelf: 'center', marginBottom: 15}}>
                    <FlatList
                        data={fieldList}
                        renderItem={({item}) => <ItemDrink item={item} onDalabaiob={()=>deleteItem(item)}/>}
